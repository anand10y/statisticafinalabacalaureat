...
st.set_page_config(layout="wide", page_title="Analiză Rezultate Elevi")
...
if uploaded_file:
    ...
    with st.sidebar:
        ...
        clase_selectate = st.multiselect("Clase disponibile:", clase, default=[])
        df_filtrat = df[df[col_clasa].isin(clase_selectate)] if clase_selectate else df.copy()

        probe_selectate = st.multiselect("Probe disponibile:", options=probe, default=[])

    # -------------------------
    # Tab-uri principale
    # -------------------------
    tab1, tab2, tab3 = st.tabs(["📊 Statistici", "📈 Grafice", "📋 Tabel complet"])
    ...
    with tab2:
        ...
        if not counts.empty:
            ...
            fig_statut.update_traces(
                ...
                textposition='inside', 
                textfont_size=22, 
                customdata=procent_custom
            )
            st.plotly_chart(fig_statut, use_container_width=True)

        # Distribuția pe probe
        for proba in probe_selectate:
            ...
            if not df_proba.empty:
                ...
                fig_proba = px.pie(
                    values=counts_proba.values, 
                    names=counts_proba.index,
                    title=f"Rezultate pentru {proba}",
                    hole=0.3, 
                    color=counts_proba.index,
                    color_discrete_map=culoare_rezultat
                )
